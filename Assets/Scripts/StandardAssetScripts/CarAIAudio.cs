...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIAudio : MonoBehaviour
    {
        ...
        public static AudioSource ai_LowAccel; // Source for the low acceleration sounds
        public static AudioSource ai_LowDecel; // Source for the low deceleration sounds
        public static AudioSource ai_HighAccel; // Source for the high acceleration sounds
        public static AudioSource ai_HighDecel; // Source for the high deceleration sounds
        ...
        private void StartSound()
        {
            ...
            ai_HighAccel = SetUpEngineAudioSource(highAccelClip);

            // if we have four channel audio setup the four audio sources
            if (engineSoundStyle == EngineAudioOptions.FourChannel)
            {
                ai_LowAccel = SetUpEngineAudioSource(lowAccelClip);
                ai_LowDecel = SetUpEngineAudioSource(lowDecelClip);
                ai_HighDecel = SetUpEngineAudioSource(highDecelClip);
            }
        ...
        private void Update()
        {
            ...
            if (m_StartedSound)
            {
                ...
                if (engineSoundStyle == EngineAudioOptions.Simple)
                {
                    ...
                    ai_HighAccel.pitch = pitch*pitchMultiplier*highPitchMultiplier;
                    ai_HighAccel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                    ai_HighAccel.volume = 1;
                }
                else
                {
                    ...
                    ai_LowAccel.pitch = pitch*pitchMultiplier;
                    ai_LowDecel.pitch = pitch*pitchMultiplier;
                    ai_HighAccel.pitch = pitch*highPitchMultiplier*pitchMultiplier;
                    ai_HighDecel.pitch = pitch*highPitchMultiplier*pitchMultiplier;
                    ...
                    ai_LowAccel.volume = lowFade*accFade;
                    ai_LowDecel.volume = lowFade*decFade;
                    ai_HighAccel.volume = highFade*accFade;
                    ai_HighDecel.volume = highFade*decFade;

                        // adjust the doppler levels
                    ai_HighAccel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                    ai_LowAccel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                    ai_HighDecel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                    ai_LowDecel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                }
            }
        }
